...
class View {
	...
	static get(id) {
		let ret = $('#'+CSS.escape(id)).val().replace(/\\n/g,'\n')
	...
	inputbig(data) {
		...
		this.f = ()=>{if (this.par) View.set(this.me,View.get(this.par[0]))}
	}
	...
	plot() {
		...
		this.f = () => {
			let frame = Frame.fromHeadedRows(View.get(this.par[0]))
			$('#'+this.me).empty()
	...
	network() {
		...
		this.f = () => Plot.plotnetwork(this.me,View.get(this.par[0]))
	}

	json2net() {
		window.godiagram = null
		this.html = `<div id='${this.me}' style='border:thin solid black;width:640px;height:400px;color:#999'>${this.me}</div>`
		this.f = () => Plot.json2net(this.me,View.get(this.par[0]))
	}
	...
	plot2layer() {
		...
		this.f = () => {
			...
			Plot.fromHeadedRows(View.get(this.par[0])).plot2layer(this.me+2)
		}
	}
