...
class ui {
	...
	static make(ids,id) {
		switch(id) {
			...
			case 'wagedata': return ui.makeselect(ids,id,Data.wage())
		}
	...
	static makeplot(ids,me) {
		...
		return () => {
			...
			if (Array.isArray(results)) {
				$('#'+me).empty()
				$('#'+me).removeAttr('style')
				if (results[0][0].length==1) Plot.factory([null,...results]).plot1 (me)
				if (results[0][0].length==2) Plot.factory([null,...results]).table2(me)
				if (results[0][0].length==3) Plot.factory([null,...results]).table3(me)
			}			
		}
	}
